<template>
    <div id="total-roll" class="fixed z-[998] top-0 left-0 right-0 bottom-0 flex flex-col bg-[rgba(36,38,45,0.5)] roll-wrapper">
        <div class="pt-[30px] ml-auto relative ml-auto inline-block">
            <button id="close-piano-roll" className="right-0 bg-[#1a1d21] hover:bg-gray-100 hover:text-[#1a1d21] transition duration-200 px-4 py-2 text-gray-100 rounded-tl-lg inline-block text-sm cursor-pointer">
                ...
                <div class="bg-[#272b33] w-full rounded p-4 flex flex-col justify-center text-center mb-4">
                    ...
                    <div class="flex w-full items-center mt-4 justify-center">
                        <button 
                            ...
                            class="back-btn-piano rounded-l-full bg-[#1e2025] text-xl text-[#8d94a5] px-3 py-1 transition duration-200 hover:bg-[#2f323c] hover:text-white">
                        ...
                        <button v-if="isPlaying"
                            ...
                            class="playstop-btn-piano rounded-r-full bg-[#1e2025] text-xl text-[#8d94a5] px-3 py-1 transition-all duration-200 hover:bg-amber-400 hover:text-white">
                            <i className="bi-pause-fill"></i>
                        </button>
                        <button v-if="!isPlaying"
                            aria-label="Play" data-balloon-pos="up" data-balloon-blunt data-balloon-nofocus
                            class="playstop-btn-piano rounded-r-full bg-[#1e2025] text-xl text-[#8d94a5] px-3 py-1 transition-all duration-200 hover:bg-green-500 hover:text-white">
            ...
            <div id="roll-panel" ref="rollPanel" class="roll-panel shadow-2xl shadow-[rgba(0,0,0,1)] overflow-x-auto overflow-y-auto bg-[#1a1d21] w-full h-full grid">
                ...
                <div class="relative" :style="{width : ((maxBatt * (16 * (divisions[0] / divisions[1]))) * unitSize) + 8 +'px'}">
                    ...
                    >
                        ...
                        <div class="bg-[#3c57dd] border border-[rgb(131,148,233)] absolute event-note" :data-noteindex="index"
...
<script setup>
    ...
    const unitSize                  = computed(() => store.getters.getUnitSizePianoRoll)
    ...
    const zoom                      = ref(store.getters.getProjectProperty("zoomPianoRoll"))
    const trackHeight               = ref(store.getters.getProjectProperty("trackHeightPianoRoll"))
    ...
    const setZoom                   = (value) => {store.commit("setProjectProperties",{zoomPianoRoll:value})}
    const setTrackHeight            = (value) => {store.commit("setProjectProperties",{trackHeightPianoRoll:value})}
    ...
    const computeBackgrounds        = () => {
        ...
        //aggiorno i listeners perchè essendo cambiate le dimensioni deve cambiare anche lo snap alla griglia del pianoroll
        addListeners()
    }
    ...
    //recupero le note dell'evento per il quale ho aperto il piano roll
    ...
    const noteKeys                    = computed(() => {
        ...
        for (let octave = octaves.value; octave >= 0; --octave) {
            notes.forEach((note,index) => {
                ...
                if(index === 11){
                    toAdd.class     = 'white-key pianorollkey key-C'+octave
                    toAdd.text      = 'C'+octave
                }else if (note.alt) {
                    toAdd.class     = 'black-key pianorollkey key-'+note.note+octave
                }else {
                    if(index === 2 || index === 4 || index === 9){
                        toAdd.class = 'white-key-large pianorollkey key-'+note.note+octave
                    }else{
                        toAdd.class = 'white-key pianorollkey key-'+note.note+octave
                    }
                }
                toReturn.push(toAdd)
            })
    ...
    const delightNotes = () => {
        let keys = document.querySelectorAll('.pianorollkey')
        keys.forEach(key => {
    ...
    const highlightNote = (noteData) => {
        ...
        highlightTimeout        = setTimeout(() => {
            let keyElement = document.getElementsByClassName("key-"+noteData.note)[0]
            if(keyElement) keyElement.classList.remove('!bg-gradient-to-br','!from-indigo-500','!to-indigo-600','!text-gray-200')
    ...
    const updateRulerRollPosition      = () => {
        if(rulerPosition.value < eventEnd){
            ...
            midiPlayer.stop()
            store.commit('togglePianoRollPlaying')
        }
    }
    ...
    const addListeners = () => {
        ...
        interact('.playstop-btn-piano').unset()
        interact('.playstop-btn-piano').on('tap', function (event) {
            event.preventDefault()
            event.stopPropagation()
            togglePlay()
        })

        interact('.back-btn-piano').unset()
        interact('.back-btn-piano').on('tap', function () {
        ...
        interact(".pianorollkey").unset()
        interact(".pianorollkey").on('down',(event) => {
        ...
        interact(".event-note").unset()
        interact('.event-note').on('tap',(event) => {event.preventDefault();event.stopPropagation()})
        interact(".event-note").on('down',(event) => {
            event.preventDefault()
            event.stopPropagation()
            
            const noteIndex       = event.target.closest('.event-note').dataset.noteindex
        ...
        interact(".event-note").on('up',(event) => {
            event.preventDefault()
            event.stopPropagation()
            const noteIndex       = event.target.closest('.event-note').dataset.noteindex
            const noteData        = realEvents.value[noteIndex]
    ...
    onMounted(() => {
        ...
        interact("#close-piano-roll").on('tap', function (event) {
            event.preventDefault()
            midiPlayer.stop()
            store.commit("setProjectProperties",{midiRollInfos : undefined})
        })
        ...
        selection.on('beforestart', evt => {
            ...
            let noteEvents = document.querySelectorAll('.event-note')
    ...
    onBeforeUnmount(() => {
        ...
        interact("#close-piano-roll").unset()
        interact(".event-note").unset()
