<template>
    <div 
        ...
        :class="{
            'animate__animated' : flash,
            'animate__shakeXShort'   : flash
        }"
        :style="{ 
...
<script setup>
    ...
    import {defineProps,ref,computed,onMounted,watch,onBeforeUnmount} from 'vue'
    ...
    const flash                     = ref(props.event.flash)
    ...
    const zoom                      = computed(() => store.getters.getProjectProperty("zoon"))
    ...
    const isSelected                = computed(() => {
        const trackIndex = store.state.project.tracks.findIndex(t => t.id === props.trackid)
        var toReturn = false
        if (trackIndex !== -1) {
            const eventIndex = store.state.project.tracks[trackIndex].events.findIndex(e => e.id === props.event.id)
            if(eventIndex >= 0){
                toReturn = store.getters.getEventProperty({trackid:props.trackid,eventid:props.event.id,property:'selected'})
            }
        }
        return toReturn
    })
    ...
    const movedeventListener = (event) => {
        if(isSelected.value && event.from != props.event.id){
            left.value  += parseInt(event.left)
            top.value   += parseInt(event.top)
        }
    }

    const endmovedListener = (event) => {
        if(isSelected.value && event.from != props.event.id){
            let trackid = props.trackid
            if(top.value != 0){
                
                const originalTrackPosition = store.getters.getTrackPosition(props.trackid)
                const newTrackPosition = originalTrackPosition + parseInt(top.value / trackHeight.value)
                const newTrackId = store.state.project.tracks[newTrackPosition].id

                if(store.getters.getTrackType(props.trackid) == store.getters.getTrackType(newTrackId)){
                    store.commit('shiftEventTrack',{
                        eventId     : props.event.id,
                        newTrackId  : newTrackId,
                        oldTrackId  : props.trackid
                    })

                    trackid = newTrackId
                }else{
                    toast("L'evento non è stato spostato perchè la traccia di destinazione non è dello stesso tipo", {
                        "theme"                 : "dark",
                        "type"                  : "error",
                        "autoClose"             : 3000
                    })   
                    ...
                    window.emitter.emit('flash',props.event.id)
                }
            }
        ...
        interact("#"+props.event.id).draggable({
            listeners : {
                ...
                start (event){
                    ...
                    selectedEvents = store.getters.getProjectProperty('selectedEvents')
                    event.preventDefault()
                    event.stopPropagation()
                    ...
                    selectedEvents.forEach((s) => {
                        const singleEvent = store.getters.getEvent(s)
                        totClamps.left  = Math.min(totClamps.left,singleEvent.left + (singleEvent.trim?singleEvent.trim.start:0))
                        totClamps.right = Math.max(totClamps.right,singleEvent.left + singleEvent.length + (singleEvent.trim?(singleEvent.trim.start+singleEvent.trim.stop):0))
                        totClamps.up    = Math.min(totClamps.up,store.getters.getTrackIndex(s.trackid))
                        totClamps.down  = Math.min(totClamps.down,(tracks.length - 1) - store.getters.getTrackIndex(s.trackid))
                    })

                    selectedEvents.forEach((s) => {
                        const singleEvent   = store.getters.getEvent(s)
                        clamps[singleEvent.id] = {
                            left    : singleEvent.left + (singleEvent.trim?singleEvent.trim.start:0) - totClamps.left,
                            right   : (maxBatt.value * 16) - singleEvent.length + (singleEvent.trim?(singleEvent.trim.start+singleEvent.trim.stop):0) - (totClamps.right - (singleEvent.left + singleEvent.length + (singleEvent.trim?(singleEvent.trim.start+singleEvent.trim.stop):0))),
                            up      : -totClamps.up,
                            down    : totClamps.down
                        }
                    })

                    //console.log(totClamps,clamps) 
                },
                move (event) {
                    ...
                    let newPosition = clamp(
                        ...
                        clamps[props.event.id].left,
                        clamps[props.event.id].right
                    )

                    //if(selectedEvents.length === 1){
                    let newTop = clamp(
                        top.value + shiftY,
                        clamps[props.event.id].up * trackHeight.value,
                        clamps[props.event.id].down * trackHeight.value
                    )
                    ...
                    const newTrackPosition = originalTrackPosition + parseInt(top.value / trackHeight.value)
                    const newTrackId = store.state.project.tracks[newTrackPosition].id
                    store.commit('setTrackSelected',newTrackId)
                    //}
                    
                    window.emitter.emit('movedevent',{
                        from    : props.event.id,
                        left    : parseInt(newPosition) - left.value,
                        top     : parseInt(newTop) - top.value
                    })
                    ...
                    setTimeout(() => {top.value = 0},1)

                    window.emitter.emit('endmovedevent',{from : props.event.id})
                    event.preventDefault()
                    event.stopPropagation()
    ...
    watch(zoom, () => { if(props.type !== "audio"){ drawMidiBackground() }})
    ...
    onMounted(() => {
        ...
        window.emitter.on('movedevent',(infos) => movedeventListener(infos))
        window.emitter.on('endmovedevent',(infos) => endmovedListener(infos))
    ...
    onBeforeUnmount(() => {
        window.emitter.off('movedevent',(infos) => movedeventListener(infos))
        window.emitter.off('endmovedevent',(infos) => endmovedListener(infos))
    })
</script>